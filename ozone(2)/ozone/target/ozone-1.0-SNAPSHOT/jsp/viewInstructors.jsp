...
<head>
    <meta charset="UTF-8">
    <title>Instructors - Ozone Institute</title>
...
<main class="max-w-5xl mx-auto py-8 px-4">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="<%=request.getContextPath()%>/instructors/add" class="bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded shadow font-semibold transition">+ Add Instructor</a>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <c:forEach var="instructor" items="${instructors}">
                <tr class="hover:bg-pink-50 transition">
                    <td class="px-4 py-3">${instructor.instructorId}</td>
                    <td class="px-4 py-3">${instructor.firstName}</td>
                    <td class="px-4 py-3">${instructor.lastName}</td>
                    <td class="px-4 py-3">${instructor.email}</td>
                    <td class="px-4 py-3">${instructor.phoneNumber}</td>
                    <td class="px-4 py-3">${instructor.departmentId}</td>
                    <td class="px-4 py-3">
                        <c:out value="${instructor.hireDate}" />
                    </td>
                    <td class="px-4 py-3 flex gap-3">
                        <a href="<%=request.getContextPath()%>/instructors/edit?id=${instructor.instructorId}" class="text-indigo-600 hover:text-indigo-900 font-medium">Edit</a>
                        <a href="<%=request.getContextPath()%>/instructors/delete?id=${instructor.instructorId}" class="text-red-600 hover:text-red-800 font-medium"
            ...
            <c:if test="${empty instructors}">
                <tr>
                    <td colspan="8" class="text-center text-gray-500 py-6">No instructors found.</td>
                </tr>
            </c:if>
            </tbody>
        </table>
