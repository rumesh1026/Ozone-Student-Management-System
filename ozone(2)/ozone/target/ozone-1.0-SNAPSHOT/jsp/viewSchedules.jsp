...
<head>
    <meta charset="UTF-8">
    <title>Schedules - Ozone Institute</title>
...
<main class="max-w-5xl mx-auto py-8 px-4">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="<%=request.getContextPath()%>/schedules/add" class="bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded shadow font-semibold transition">+ Add Schedule</a>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <c:forEach var="schedule" items="${schedules}">
                <tr class="hover:bg-cyan-50 transition">
                    <td class="px-4 py-3">${schedule.scheduleId}</td>
                    <td class="px-4 py-3">${schedule.courseId}</td>
                    <td class="px-4 py-3">${schedule.dayOfWeek}</td>
                    <td class="px-4 py-3">${schedule.startTime}</td>
                    <td class="px-4 py-3">${schedule.endTime}</td>
                    <td class="px-4 py-3">${schedule.location}</td>
                    <td class="px-4 py-3 flex gap-3">
                        <a href="<%=request.getContextPath()%>/schedules/edit?id=${schedule.scheduleId}" class="text-blue-600 hover:text-blue-900 font-medium">Edit</a>
                        <a href="<%=request.getContextPath()%>/schedules/delete?id=${schedule.scheduleId}" class="text-red-600 hover:text-red-800 font-medium"
